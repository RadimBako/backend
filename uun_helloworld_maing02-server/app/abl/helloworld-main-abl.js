...
const Errors = require("../api/errors/helloworld-main-error.js");
...
const logger = LoggerFactory.get("HelloworldMainAbl");

class HelloworldMainAbl {
  constructor() {
    this.validator = Validator.load();
    this.dao = DaoFactory.getDao(Schemas.HELLOWORLD_MAIN);
  ...
  async init(uri, dtoIn, session) {
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "UunHelloworld",
        typeCode: "uun-helloworld-maing02",
        location: location,
    ...
    let helloworldMain;
    try {
      helloworldMain = await this.dao.create(uuObject);
    } catch (e) {
      throw new Errors.Init.HelloworldDaoCreateFailed({ uuAppErrorMap }, e);
    }
...
module.exports = new HelloworldMainAbl();
